...
const getSizeHTML = (size) => {
  ...
  return [
    ...
    '</svg>',
    ` <span class="num text-emphasized" data-github-repo-size=${size}>${humanReadableSize.size} ${humanReadableSize.measure}</span>`,
    '</li>',
    `<li>
...
const onMeasureChanged = (event) => {
  ...
  // update all data
  document.querySelectorAll('[data-github-repo-size]').forEach(el => {
      el.textContent = getHumanReadableSize(el.getAttribute('data-github-repo-size'))
  })
}
...
const checkForRepoPage = async () => {
  ...
  for (let item of items) {
    ...
    td.innerHTML = `<span class="css-truncate css-truncate-target github-repo-size-td" ${isNaN(t) ? '' : `data-github-repo-size=${t}`}>${label}</span>`
...
const loadFolderSizes = async () => {
  ...
  for (const folderSize of folderSizes) {
    ...
    folderSize.setAttribute('data-github-repo-size', t)
